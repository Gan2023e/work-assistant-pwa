...
const parseInvoicePDF = (text) => {
  ...
  try {
    ...
    // 金额解析 - 优先匹配"小写"后面的数字，避免匹配表格中的金额和发票号码
    const amountPatterns = [
      // 第一优先级：匹配"价税合计（小写）"后面的数字
      /价税合计[^¥]*小写[^0-9]{0,20}¥?\s*([\d,]{1,15}\.\d{2})/,
      /价税合计[^¥]*小写[^0-9]{0,20}¥?\s*([\d,]{1,15}\.?\d{0,2})/,
      // 第二优先级：匹配"小写"后面带¥符号的数字
      /小写[^0-9]{0,20}¥\s*([\d,]{1,15}\.\d{2})/,
      /小写[^0-9]{0,20}¥\s*([\d,]{1,15}\.?\d{0,2})/,
      // 第三优先级：匹配"小写"后面的数字（不要求¥符号）
      /小写[^0-9]{0,20}([\d,]{1,15}\.\d{2})/,
      /小写[^0-9]{0,20}([\d,]{1,15}\.?\d{0,2})/
    ];
    ...
    for (const pattern of amountPatterns) {
      ...
      if (match) {
        ...
        // 验证金额的合理性：不能是发票号码，长度不能超过10位，必须是有效的金额
