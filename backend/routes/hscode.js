  ...
  try {
    ...
    if (ossDeleteResult) {
      if (ossDeleteResult.success) {
        response.message = '删除成功（OSS文件已删除）';
      } else {
        response.message = '记录删除成功，但OSS文件删除失败';
        response.ossDeleteResult = ossDeleteResult;
      }
    }
