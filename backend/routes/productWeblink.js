...
async function sendCpcTestApprovedNotification(approvedCount) {
  try {
    ...
    if (response.data.errcode === 0) {
      console.log('CPC测试申请通过钉钉通知发送成功');
    } else {
      console.error('CPC测试申请通过钉钉通知发送失败:', response.data);
    }
  } catch (error) {
...
router.post('/generate-uk-data-sheet', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.log(`📍 找到扩展列位置 - brand_name: ${brandNameCol}, manufacturer: ${manufacturerCol}, external_product_id: ${externalProductIdCol}, external_product_id_type: ${externalProductIdTypeCol}`);
...
router.post('/generate-other-site-datasheet', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    // 步骤2: 如果是阿联酋站点，查询英国站点的ASIN数据
    let ukAsinMap = new Map();
    if (actualCountry === 'AE') {
      try {
        const ListingsSku = require('../models/ListingsSku');
        const ukListings = await ListingsSku.findAll({
          where: {
            site: 'www.amazon.co.uk'
          },
          attributes: ['seller-sku', 'asin1']
        });
        
        ukListings.forEach(listing => {
          if (listing['seller-sku'] && listing.asin1) {
            ukAsinMap.set(listing['seller-sku'], listing.asin1);
          }
        });
      } catch (error) {
        console.error('❌ 查询英国站点ASIN数据失败:', error);
        // 不中断流程，继续执行
      }
    }

    // 步骤3: 处理数据并保存到数据库，同时准备填写到Excel
    ...
    processedRecords.forEach((record, index) => {
      ...
      // 处理external_product_id和external_product_id_type字段
      if (actualCountry === 'AE' && externalProductIdCol !== -1) {
        // 阿联酋站点：只有子SKU才填写ASIN，母SKU不填写
        const childSku = recordData.item_sku;
        const isParentSku = recordData.parent_child === 'Parent';
        
        if (isParentSku) {
          data[currentRowIndex][externalProductIdCol] = '';
        } else {
          if (childSku && ukAsinMap.has(childSku)) {
            data[currentRowIndex][externalProductIdCol] = ukAsinMap.get(childSku);
          } else {
            data[currentRowIndex][externalProductIdCol] = '';
          }
        }
      } else if (externalProductIdCol !== -1) {
        // 其他站点：使用原有逻辑
        data[currentRowIndex][externalProductIdCol] = recordData.external_product_id || '';
      }
      
      if (externalProductIdTypeCol !== -1) {
        if (actualCountry === 'AE') {
          // 阿联酋站点：只有子SKU才填写类型，母SKU不填写
          const childSku = recordData.item_sku;
          const isParentSku = recordData.parent_child === 'Parent';
          
          if (isParentSku) {
            data[currentRowIndex][externalProductIdTypeCol] = '';
          } else if (childSku && ukAsinMap.has(childSku)) {
            data[currentRowIndex][externalProductIdTypeCol] = 'ASIN';
          } else {
            data[currentRowIndex][externalProductIdTypeCol] = '';
          }
        } else {
          // 其他站点：使用原有逻辑
          data[currentRowIndex][externalProductIdTypeCol] = recordData.external_product_id_type || '';
        }
      }
      ...
      // 填写seasons字段，当feed_product_type为Backpack时自动填写
      if (seasons1Col !== -1) {
      ...
      if (depthFrontToBackCol !== -1) {
        ...
        // 美国站点特殊处理：如果单位是Centimeters/Centimetres，转换为英寸
      ...
      if (depthFrontToBackUnitOfMeasureCol !== -1) {
        ...
        console.log(`🔧 处理depth_front_to_back_unit_of_measure: 原值="${depthUnit}", 目标国家="${actualCountry}"`);
        // 美国站点特殊处理：转换为Inches
        if (actualCountry === 'US') {
          depthUnit = 'Inches';
          console.log(`🔧 US 单位转换: "${recordData.depth_front_to_back_unit_of_measure}" -> "${depthUnit}"`);
        }
...
CN
${process.env.MANUFACTURER_PHONE}`;
        } else {
          // 其他站点保持原有逻辑
  ...
  try {
    ...
    records.forEach((record, index) => {
      ...
      if (depthFrontToBackUnitOfMeasureCol !== -1) {
        ...
        console.log(`🔧 [mapDataToTemplateXlsx] 处理depth_front_to_back_unit_of_measure: 原值="${depthUnit}", 目标国家="${country}"`);
        // 美国站点特殊处理：转换为Inches
        if (country === 'US') {
          depthUnit = 'Inches';
          console.log(`🔧 [mapDataToTemplateXlsx] US 单位转换: "${data.depth_front_to_back_unit_of_measure}" -> "${depthUnit}"`);
        }
        // 加拿大站点特殊处理：Inches转换为Centimeters
      ...
      if (depthWidthSideToSideUnitOfMeasureCol !== -1) {
        ...
        console.log(`🔧 [mapDataToTemplateXlsx] 处理depth_width_side_to_side_unit_of_measure: 原值="${widthUnit}", 目标国家="${country}"`);
        // 加拿大站点特殊处理：Inches转换为Centimeters
      ...
      if (depthHeightFloorToTopUnitOfMeasureCol !== -1) {
        ...
        console.log(`🔧 [mapDataToTemplateXlsx] 处理depth_height_floor_to_top_unit_of_measure: 原值="${heightUnit}", 目标国家="${country}"`);
        // 加拿大站点特殊处理：Inches转换为Centimeters
