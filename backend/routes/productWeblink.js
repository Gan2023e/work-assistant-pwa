    ...
    if (data[fieldName] !== undefined && data[fieldName] !== null && data[fieldName] !== '') {
      ...
      if (fieldConfig.type === 'string' && fieldConfig.maxLength) {
        ...
        if (typeof value === 'string' && value.length > fieldConfig.maxLength) {
          ...
          console.warn(`?? 字段 ${fieldName} 长度超限，已截断: 原长度${data[fieldName].length} -> 截断后${value.length}`);
        } else if (typeof value !== 'string') {
          // 非字符串转换为字符串
          value = String(value);
          if (value.length > fieldConfig.maxLength) {
            value = value.substring(0, fieldConfig.maxLength - 3) + '...';
            console.warn(`?? 字段 ${fieldName} 转换为字符串后长度超限，已截断: ${value.length}`);
          }
        }
  ...
  try {
    ...
    try {
      ...
      if (!hasExistingExtractedData && (extractedData.styleNumber || extractedData.recommendAge)) {
        ...
        console.log(`?? SKU ${record.parent_sku} 已有提取信息，跳过重复提取`);
      }
  ...
  try {
    ...
    if (fileToDelete.objectName) {
      try {
        ...
        console.warn(`?? OSS文件删除失败: ${fileToDelete.objectName}`, ossError.message);
        // 继续执行数据库删除，即使OSS删除失败
      }
    }
...
router.post('/amazon-templates/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn('?? 保存模板信息到数据库失败:', dbError.message);
      // 不阻断上传流程，只是警告
    }
...
router.delete('/amazon-templates/:objectName*', async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn('?? 模板文件备份失败，继续删除操作:', backupError.message);
    }
    ...
    try {
      ...
      if (deletedCount > 0) {
        ...
        console.warn('?? 数据库中未找到对应的模板记录');
      }
    } catch (dbError) {
      console.warn('?? 从数据库删除模板记录失败:', dbError.message);
      // 不阻断删除流程，只是警告
    }
  ...
  try {
    ...
    for (const row of dataRows) {
      ...
      if (rowData.parent_child === 'Parent' && rowData.item_sku && rowData.item_sku.length > 2) {
        ...
        rowData.original_parent_sku = rowData.item_sku.substring(2);
        console.warn(`?? 记录缺少parent_child信息，使用item_sku生成original_parent_sku: ${rowData.item_sku} -> ${rowData.original_parent_sku}`);
      }
      ...
      try {
        ...
        console.warn(`?? 保存记录失败: ${JSON.stringify(filteredData)}, 错误: ${error.message}`);
...
function mapDataToTemplateXlsx(templateData, records, country) {
  try {
    ...
    if (missingColumns.length > 0) {
      console.warn(`?? 模板中缺少以下列: ${missingColumns.join(', ')}`);
    }
  ...
  try {
    ...
    for (const row of dataRows) {
      ...
      if (rowData.parent_child === 'Parent' && rowData.item_sku && rowData.item_sku.length > 2) {
        ...
        // 兼容处理：如果没有parent_child信息，使用原有逻辑
        rowData.original_parent_sku = rowData.item_sku.substring(2);
        console.warn(`?? 批量记录缺少parent_child信息，使用item_sku生成original_parent_sku: ${rowData.item_sku} -> ${rowData.original_parent_sku}`);
      }
  ...
  try {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      if (record.parent_child === 'Parent' && record.item_sku && record.item_sku.length > 2) {
        ...
        // 兼容处理：如果没有parent_child信息，使用原有逻辑
        record.original_parent_sku = record.item_sku.substring(2);
        console.warn(`?? 批量处理记录缺少parent_child信息，使用item_sku生成original_parent_sku: ${record.item_sku} -> ${record.original_parent_sku}`);
      }
    ...
    inventorySkus.forEach((inventory, index) => {
      ...
      if (columnIndexes['external_product_id'] !== undefined) {
        if (listingInfo && listingInfo.asin) {
          ...
          console.log(`??  跳过ASIN填写: ${childSku}, amzSku: ${amzSkuInfo?.amz_sku || 'N/A'}`);
          // 不填写空值，直接跳过
        }
      }
      ...
      if (columnIndexes['standard_price'] !== undefined) {
        if (listingInfo && listingInfo.price) {
          ...
          console.log(`??  跳过价格填写: ${childSku}, amzSku: ${amzSkuInfo?.amz_sku || 'N/A'}`);
          // 不填写空值，直接跳过
        }
      }
  ...
  try {
    ...
    const insertPromises = mappings.map(async (mapping) => {
      try {
        ...
        if (existing) {
          console.log(`??  映射已存在，跳过: ${mapping.amz_sku} (${mapping.site})`);
